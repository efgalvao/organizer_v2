...
RSpec.describe CategoryServices::FetchExpensesByCategory do
  ...
  let!(:expense) { create(:expense, account: account, category_id: category.id, date: Date.current) }
  ...
  describe '#call' do
    ...
    context 'when filtering by the current month' do
      let!(:past_expense) { create(:expense, account: account, category_id: category.id, date: 1.month.ago) }
