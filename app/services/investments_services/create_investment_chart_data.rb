module InvestmentsServices
  class CreateInvestmentChartData
    ...
    def investment
      @investment ||= InvestmentRepository.find(investment_id)
    end

    def fetch_positions
      summary = { shares: {}, amounts: {} }
      InvestmentRepository.positions_for(investment).map do |position|
    ...
    def fetch_negotiations
      summary = { shares: {}, amounts: {} }
      InvestmentRepository.negotiations_for(investment).map do |negotiation|
    ...
    def fetch_earnings
      ...
      InvestmentRepository.dividends_for(investment).map do |dividend|
        summary[:dividends][dividend.date.strftime('%m/%d/%Y').to_s] = dividend.amount
      end
      InvestmentRepository.interests_for(investment).map do |interest|
